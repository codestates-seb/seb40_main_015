...
public class ChatService {
	...
	public ChatRoomResponse findAllChats(Long roomId) {
		...
		List<ChatResponse> collect = chats.stream().map(ChatResponse::of).collect(Collectors.toList());
	...
	public List<LastChatResponse> findAllLastChats(Long memberId) {
		...
		return allLastChats.stream()
			.map(chat -> LastChatResponse.of(chat.getRoom().getPartner(memberId), chat))
			.collect(Collectors.toList());
	}

	@Transactional
	public void save(Long roomId, ChatMessageDto message, LocalDateTime now) {
		Member sender = memberService.getById(message.getSenderId());
		Member receiver = memberService.getById(message.getReceiverId());
