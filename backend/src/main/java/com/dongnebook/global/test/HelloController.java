...
public class HelloController {
	...
	public long BulkMemberInsert() {
		...
		List<Member> list = IntStream.range(0, 1000000)
			.mapToObj(i -> makeMember(locations, i))
	...
	public long BulkBookInsert() {
		List<Location> locations = getLocations();
		List<Book> list = IntStream.range(0, 1000000)
			.mapToObj(i -> Book.create(BookRegisterRequest.builder().author("asdf"+i)
					...
					.rentalFee(i)
					.title("asdf"+i)
				.build(),locations.get(i%4), Member.builder().build()))
			.collect(Collectors.toList());
		long startTime = System.currentTimeMillis();
		jdbcTemplate.batchUpdate(
	...
	private Member makeMember(List<Location> locations, int i) {
